...
namespace cremeCoffeeBurgett.Models
{
    ...
    public class RouteDictionary : Dictionary<string, string>
    {
        ...
        public string CountryFilter {
            get => Get(nameof(BeansGridDTO.Country))?.Replace(FilterPrefix.Country, "");
            set => this[nameof(BeansGridDTO.Country)] = value;
        }

        public string PriceFilter {
            get => Get(nameof(BeansGridDTO.Price))?.Replace(FilterPrefix.Price, "");
            set => this[nameof(BeansGridDTO.Price)] = value;
        }

        public string OriginFilter {
            get
            {
                string s = Get(nameof(BeansGridDTO.Origin))?.Replace(FilterPrefix.Origin, "");
                int index = s?.IndexOf('-') ?? -1;
                return (index == -1) ? s : s.Substring(0, index);
            }
            set => this[nameof(BeansGridDTO.Origin)] = value;
        }

        public void ClearFilters() =>
            CountryFilter = PriceFilter = OriginFilter = BeansGridDTO.DefaultFilter;
